...
export function CamerasPage() {
  ...
  async function saveCamera(event: FormEvent) {
    ...
    if (draft.camera_type === "ip_webcam" && !draft.snapshot_url.trim()) {
      setToast({ tone: "error", title: "Snapshot URL required", message: "IP Webcam cameras need an HTTP snapshot URL." });
      return;
    }
    ...
    <div className="grid gap-6">
      <section className="surface-strong p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-white">Configure attendance cameras</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Connect IP Webcam snapshot URLs, RTSP streams, local webcams, or manual capture sources.</p>
          </div>
      ...
      >
        <form id="camera-form" className="grid gap-4 md:grid-cols-2" onSubmit={(event) => void saveCamera(event)}>
          ...
          <label className="grid gap-1 text-sm">Camera type<select className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-slate-900" value={draft.camera_type} onChange={(event) => setDraft({ ...draft, camera_type: event.target.value as Camera["camera_type"] })}><option value="ip_webcam">IP Webcam</option><option value="rtsp">RTSP</option><option value="webcam">Webcam</option><option value="manual">Manual</option></select></label>
