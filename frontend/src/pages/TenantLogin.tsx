...
import { loginTenantAdmin } from "@/services/tenantAuth";
import { formatTenantLabel } from "@/utils/tenantRouting";

function redirectForRole(role: string) {
  if (role === "SUPER_ADMIN") return "/admin/dashboard";
  if (role === "TENANT_ADMIN") return "/tenant-admin/dashboard";
  if (role === "TENANT_USER") return "/user/dashboard";
  if (role === "SECURITY_GUARD") return "/user/security";
  if (role === "RECEPTIONIST") return "/user/reception";
  if (role === "ATTENDANCE_OPERATOR") return "/user/attendance";
  if (role === "CAMERA_OPERATOR") return "/user/cameras";
  if (role === "MANAGER") return "/user/security";
...
export function TenantLogin() {
  ...
  const organizationName = useMemo(() => {
    const queryValue = searchParams.get("org")?.trim() || searchParams.get("tenant")?.trim();
    if (queryValue) return queryValue;
    if (organizationSlug) return formatTenantLabel(decodeURIComponent(organizationSlug));
    return "";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const session = await loginTenantAdmin(form.email, form.password);
      ...
      <Card className="w-full border-white/10 bg-slate-950/85 p-6 shadow-[0_24px_70px_rgba(2,6,23,0.42)] backdrop-blur-xl sm:p-8">
        <div className="mb-8 flex flex-col items-center gap-4 text-center">
          ...
          <div className="space-y-1">
            ...
            <div className="text-sm text-slate-400">Use the tenant admin credentials provided by your super admin.</div>
          </div>
        </div>
