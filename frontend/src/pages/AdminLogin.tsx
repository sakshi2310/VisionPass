...
function redirectForRole(role: string) {
  return role === "SUPER_ADMIN" ? "/admin/dashboard" : "/admin/login";
}
...
export function AdminLogin() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const session = await loginSuperAdmin(form.email, form.password);
      ...
      <Card className="w-full border-white/10 bg-slate-950/85 p-6 shadow-[0_24px_70px_rgba(2,6,23,0.42)] backdrop-blur-xl sm:p-8">
        <div className="mb-8 flex flex-col items-center gap-4 text-center">
          ...
          <div className="space-y-1">
            <div className="text-sm font-medium text-white">Super Admin Login</div>
            <div className="text-sm text-slate-400">Platform administrators sign in here.</div>
          </div>
        </div>
