...
def login(payload: LoginRequest, db: Session = Depends(database_session)) -> AuthResponse:
    logger.info(f'>>> TENANT USER LOGIN ATTEMPT -- email: {payload.email}')
    try:
        tenant, user = authenticate_tenant_user_login(db, payload.email, payload.password)
    ...
    logger.info(f'LOGIN SUCCESS -- Tenant User: "{user.full_name}" | Tenant: "{tenant.name}"')
    return AuthResponse(
